...
<div class="vocab-card" id="card-${v.type}-${v.id}" style="position: relative; overflow: hidden;"> 
    ...
    <div id="overlay-${k}" class="trick-overlay" style="display:none; position:absolute; top:0; left:0; width:100%; height:100%; background:var(--card); z-index:100; flex-direction:column; padding:15px; border-radius:20px;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
            ...
            <span onclick="toggleTrick('${k}')" style="cursor:pointer; color:#ef4444; font-weight:bold;">✖</span>
        </div>
        <textarea id="trick-input-${k}" style="flex:1; width:100%; background:rgba(129,140,248,0.08); border:1px dashed var(--p); padding:12px; border-radius:12px; resize:none;" placeholder="Trick...">${savedTrick}</textarea>
        <button style="width:100%; padding:12px; background:var(--p); color:white; border:none; border-radius:10px; cursor:pointer; font-weight:bold; margin-top:10px;" onclick="handleSaveTrick('${k}')">Save to Cloud</button>
    </div>
    <div class="v-btns">
