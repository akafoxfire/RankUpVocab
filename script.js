...
window.handleSaveTrick = function(k) {
    ...
    if (!trickText) return alert("Please enter text!");
            ...
            return `<div class="vocab-card" id="card-${v.type}-${v.id}" style="position: relative;"> 
                ...
                <div id="overlay-${k}" class="trick-overlay" style="display:none; position:absolute; top:0; left:0; width:100%; height:100%; background:var(--card); z-index:10; flex-direction:column; padding:15px; border-radius:20px;">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <span style="font-weight:700;">My Trick:</span>
                        <span onclick="toggleTrick('${k}')" style="cursor:pointer; color:red;">✖</span>
                    </div>
                    <textarea id="trick-input-${k}">${savedTrick}</textarea>
...
const quiz = {
    ...
    finish() {
        ...
        document.getElementById('res-actions-div').innerHTML = `<button onclick="quiz.retryMistakes()" class="btn-retry">Retry Mistakes</button><button onclick="router('study')" class="btn-home">Home</button>`;
    },
    retryMistakes() {
...
window.openAboutModal = function() { document.getElementById('aboutModal').style.display = 'flex'; };
window.closeAboutModal = function() { document.getElementById('aboutModal').style.display = 'none'; };
